...
void main(){
	...
	vec3 diffuse_color;
	if(frag_but1==0.0) diffuse_color=vec3(0.35 ,0.3 ,0.15);
	else if(frag_but1==1.0)diffuse_color = texture(TextureMap, texcoord).rgb;// Get diffuse color
	...
	if(frag_but3==0.0)
	{
		if(frag_but2==1.0) specular = 0.5 *vec3(1,1,1) * pow(max(0, dot(newnor, half_vector)), 10);
		else if(frag_but2==0.0) specular = 0.01 *vec3(1,1,1) * pow(max(0, dot(newnor, half_vector)), 0.2);		
	}
	else if(frag_but3==1.0)
	{	
		if(frag_but2==1.0) specular = 0.5* map_spec*vec3(1,1,1) * pow(max(0, dot(newnor, half_vector)), 10);
		else if(frag_but2==0.0) specular = 0.01* map_spec*vec3(1,1,1) * pow(max(0, dot(newnor, half_vector)), 0.2);
	}
